...
Command line args:
    ...
    --model choice between categorical naive Bayes ('mnb') and random forest classification ('rf')
...
parser.add_argument(
    ...
    type=str,
    default="mnb",
    help="categorical naive Bayes ('mnb') or random forest classification ('rf')?",
)
...
def main(args):
    ...
    if args.model == "mnb":
        ...
        model_details.loc[len(model_details)] = [args.data, "mnb", args.target, acc]
        model_details.to_csv(
            f"examples/experiments_cg/results/discrete/true_amat/{args.data}/model_details_mnb.csv", index=False
        )
    else:
        # fit model
    ...
    # CG est Sage run with same orderings as SAGE run
    ...
    # Separate CG est SAGE run with convergence detection
