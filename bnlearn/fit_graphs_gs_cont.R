# Bayesian network structure learning using grow-shrink algorithm (as implemented in bnlearn)
# setwd("~/thesis_code")
dir.create("bnlearn/results")
dir.create("bnlearn/results/gs")
dir.create("bnlearn/results/gs/est_amat")
...
for (i in graphs_cont){
  ...
  for (sample_size in sample_sizes){
    ...
    if (i == "healthcare"){
      # as.factor() required for bnlearn.gs()
      for (j in c("A", "C", "H")){
        df[,j] <- as.factor(df[,j]) 
      }
    }
    ...
    # adjacency matrix
    adj_mat <- amat(dag)
    amat_file <- paste("bnlearn/results/gs/est_amat/", i, "_", sample_size, "_obs.csv", sep="")
    write.csv(adj_mat, file=amat_file, row.names = FALSE)
  }
}

# save table
write.csv(table,"bnlearn/results/gs/runtime_data_cont.csv", row.names = FALSE)
